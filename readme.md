...
For each supported language, there is a class implementing interface [SymbolExtractor](src/main/java/com/gitplex/jsymbol/SymbolExtractor.java). The implementation parses source of target language and return a list of symbols representing static structure of the source. GitPlex then uses this list to do below things:

1. Construct source outline. Root nodes of the outline are those symbols with a _null_ [parent](src/main/java/com/gitplex/jsymbol/Symbol.java?mark=30.0-31.0). To determine children of a particular symbol, GitPlex loops the list to find symbols with [parent](src/main/java/com/gitplex/jsymbol/Symbol.java?mark=30.0-31.0) set to that symbol
1. Index names of [non-local](src/main/java/com/gitplex/jsymbol/Symbol.java?mark=86.0-87.0) to do symbol search and cross reference later
...
```
The [JavaExtractor](src/main/java/com/gitplex/jsymbol/java/JavaExtractor.java) will extract below list of symbols:

1. A [compilation unit symbol](src/main/java/com/gitplex/jsymbol/java/symbols/CompilationUnitSymbol.java) representing package _com.example.logger_
1. A [type symbol](src/main/java/com/gitplex/jsymbol/java/symbols/TypeSymbol.java) representing class _Logger_, with parent symbol set to compilation unit _com.example.logger_
1. A [field symbol](src/main/java/com/gitplex/jsymbol/java/symbols/FieldSymbol.java) representing field _name_, with parent symbol set to type definition _Logger_
1. A [method symbol](src/main/java/com/gitplex/jsymbol/java/symbols/MethodSymbol.java) representing method _getName()_, with parent symbol set to type definition _Logger_
1. A [method symbol](src/main/java/com/gitplex/jsymbol/java/symbols/MethodSymbol.java) representing method _setName(String)_, with parent symbol set to type definition _Logger_
1. A [method symbol](src/main/java/com/gitplex/jsymbol/java/symbols/MethodSymbol.java) representing method _log(String)_, with parent symbol set to type definitio*n _Logger_
...
1. Displays source outline as:
  ...
  > ![java-symbols.png](doc/img/java-symbols.png)
...
1. Since we only need to use grammar navigation feature of this plugin, we deactivate parser auto-generating as below (GitPlex pom file is already configured to auto-generate parsers):
  ...
  ![eclipse-antlr-deactivate-tool.png](doc/img/eclipse-antlr-deactivate-tool.png)
...
1. Import the cloned project as Maven project into Eclipse
  ...
  ![eclipse-import-maven.png](doc/img/eclipse-import-maven.png)
...
To add support for a new language, create a package of that language in JSymbol. For instance, the package [com.gitplex.jsymbol.c](src/main/java/com/gitplex/jsymbol/c) contains everything needed to extract symbols from C language:

1. Class [CExtractor](src/main/java/com/gitplex/jsymbol/c/CExtractor.java) is entry point of symbol extraction for C language. It is extend from [AbstractSymbolExtractor](src/main/java/com/gitplex/jsymbol/AbstractSymbolExtractor.java?mark=7.22-7.45). All extractor instance should be **thread safe**.
1. The package [com.gitplex.jsymbol.c.symbols](src/main/java/com/gitplex/jsymbol/c/symbols) contains symbols defined for C language. They all extended from the base class [CSymbol](src/main/java/com/gitplex/jsymbol/c/symbols/CSymbol.java) containing some common logic/data for C symbols
1. The package [com.gitplex.jsymbol.c.symbols.ui](src/main/java/com/gitplex/jsymbol/c/symbols/ui) contains some [Wicket](http://wicket.apache.org/) component created to render the symbols. You do not need to be familiar with Wicket, as these components are very simple and straightforward. Just create yours in the same way if necessary. 
1. The package [com.gitplex.jsymbol.c.symbols.ui.icon](src/main/java/com/gitplex/jsymbol/c/symbols/ui/icon) contains icons used by the symbols. 
1. Class [CExtractorTest](src/test/java/com/gitplex/jsymbol/c/CExtractorTest.java) to verify correctness of the extractor.
...
For languages using ANTLR generated parsers, you may put the grammar file under the language package. For instance, package [com.gitplex.jsymbol.c](src/main/java/com/gitplex/jsymbol/c) contains [CDeclarationLexer.g4](src/main/java/com/gitplex/jsymbol/c/CDeclarationLexer.g4) and [CDeclarationParser.g4](src/main/java/com/gitplex/jsymbol/c/CDeclarationParser.g4). Eclipse will automatically generate lexer and parser classes under source folder _target/generated-source/antlr4_ when grammar file is added or updated. Sometimes Eclipse has issues compiling the grammar and may complain some odd errors, which can normally be fixed by [updating the project](#updateproject). Often you need to adjust the grammar for below reasons:
...
![web server](doc/img/debug-webserver.png)
